...
// 定义一个包含多个汉字的指针数组
const uint8* hanzi[15] = {    
     (const uint8*)banshou			,
     (const uint8*)dianlaotie		,
     (const uint8*)shoudianzuan	,
     (const uint8*)juanchi			,
     (const uint8*)luosidao			,
     (const uint8*)qianzi				,
     (const uint8*)shiboqi			,
     (const uint8*)wanyongbiao	,
     (const uint8*)dayinji			,
     (const uint8*)jianpan			,
     (const uint8*)shouji				,
     (const uint8*)shubiao			,
     (const uint8*)erji					,
     (const uint8*)xianshiqi		,
     (const uint8*)yinxiang			,
